...
//        $this->CI->load->library('database');
    ...
    public function setDateRange($start_date = '', $end_date = '', $column_name = '')
    {
        $this->query .= ' WHERE ' . $column_name . ' >= "' . $start_date . '" AND '.$column_name .'<= "' . $end_date . '"';
    }

    public function setSpecificValue($value, $searchColumn, $where = true)
    {
        $status = $where == true ? ' WHERE ' : ' AND ';
        $this->query .= "$status $searchColumn = '$value'";
    }
