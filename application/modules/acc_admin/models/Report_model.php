...
WHERE type='Expense' And trans_date 
...
//For Income
            if($maxIncome>0){
                ...
                $income[$day] = array("amount" => 0, "date" => $date1, "m_name" => date('F'));
            }
//End Income
...
LAST_DAY('".$date."')")->row();
    ...
    public function financialBalance(){
        $query=$this->db->query("SELECT a.opening_balance,a.note,a.accounts_id, a.accounts_name as account,(SELECT bal FROM transaction
...
//Day wise and date wise Income Report
    ...
    public function getIncomeReport($from_date,$to_date,$group=''){
        if($group=="group"){
            $query = $this->db->query("SELECT trans_date,sum(amount) as amount FROM
	transaction WHERE type='Income' AND trans_date between '".$from_date."'
	AND '".$to_date."' GROUP BY trans_date");
            return $query->result();
        }else{
...
//Transfer Report
    ...
    public function getTransferReport($from_date,$to_date){
        $query = $this->db->query("SELECT * FROM transaction WHERE type='Transfer' 
...
//Category Report
    public function getCategoryReport($from_date,$to_date,$cat){
        $query = $this->db->query("SELECT * FROM transaction WHERE category='".$cat."'
AND trans_date between '".$from_date."' AND '".$to_date."'");
        return $query->result();
    }
...
//Payer Report
    public function getPayerReport($from_date,$to_date,$payer){
        $query = $this->db->query("SELECT * FROM transaction WHERE payer='".$payer."'
AND trans_date between '".$from_date."' AND '".$to_date."'");
        return $query->result();
    }
...
//Payee Report
    public function getPayeeReport($from_date,$to_date,$payee){
        $query = $this->db->query("SELECT * FROM transaction WHERE payee='".$payee."' AND trans_date between '".$from_date."' AND '".$to_date."'");
        return $query->result();
    }
