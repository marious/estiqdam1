...
class MyUploader
{
    ...
    public function upload($renameDuplicateds = true)
    {
        $this->renameDuplicates = $renameDuplicateds;
        ...
        if (is_array($uploaded['name'])) {
            foreach ($uploaded['name'] as $key => $value) {
                ...
                if ($this->checkFile($currentFile)) {
                    $filename = $this->moveFile($currentFile);
                }
            }
        } else {
            if ($this->checkFile($uploaded)) {
                $filename = $this->moveFile($uploaded);
            }
        }
        if (isset($filename)) {
            return $filename;
    ...
    protected function getErrorMessage($file)
    {
        switch($file['error']) {
            case 1:
            case 2:
                $this->messages['big_file'] = $file['name'] . ' is too big: (max: ' .
                    self::convertFromBytes($this->maxSize) . ').';
                break;
            case 3:
                $this->messages['partialy_uploaded'] = $file['name'] . ' was only partially uploaded.';
                break;
            case 4:
                $this->messages['not_submitted'] = 'No file submitted.';
                break;
            default:
                $this->messages['problem_happen'] = 'Sorry, there was a problem uploading ' . $file['name'];
                break;
        }
    }
    ...
    {
        if ($file['size'] == 0) {
            $this->messages['name_empty'] = $file['name'] . ' is empty.';
            return false;
        } elseif ($file['size'] > $this->maxSize) {
            $this->messages['max_size'] = $file['name'] . ' exceeds the maximum size for a file ('
                . self::convertFromBytes($this->maxSize) . ').';
            return false;
        } else {
            return true;
    ...
    protected function checkType($file)
    {
        if (in_array($file['type'], $this->permittedTypes)) {
            ...
            $this->messages['not_permitted'] = $file['name'] . ' is not permitted type of file.';
            return false;
        }
    }
    ...
    protected function moveFile($file)
    {
        ...
        if ($success) {
            ...
            $this->messages['error'] = 'Could not upload ' . $file['name'];
        }
